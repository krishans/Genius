...
function App() {
  ...
  const handleAnswer = (correct: boolean) => {
    ...
    if (correct) {
      ...
      updateActiveProfile({
        totalScore: activeProfile.totalScore + 1,
        difficulty: newDiff,
        highScore: isTimeMode && newScore > activeProfile.highScore ? newScore : activeProfile.highScore
      });

      setFeedback('correct');
      setTimeout(() => setChallenge(MathEngine.generate(activeProfile.grade, newDiff)), 600);
    } else {
      ...
      {!activeId ? (
        ...
        <div className="w-full max-w-lg">
          <div className="flex justify-between items-center mb-6">
            ...
            {streak >= 3 && <div className="bg-orange-500 text-white px-3 py-1 rounded-full font-black text-xs animate-bounce">🔥 {streak}</div>}
      ...
      <AnimatePresence>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 bg-blue-600/95 z-50 flex flex-col items-center justify-center p-6">
            <motion.div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-sm border-b-8 border-blue-200">
              ...
              <div className="space-y-4">
                <input value={newName} onChange={e => setNewName(e.target.value)} className="w-full p-4 bg-gray-50 rounded-xl border-2 border-gray-100 font-bold" placeholder="Kid Name" />
                ...
                <div className="flex pt-4"><button onClick={() => setIsAddingKid(false)} className="flex-1 font-black text-gray-400">Cancel</button><button onClick={addProfile} className="bg-blue-600 text-white px-8 py-4 rounded-xl font-black shadow-lg">GO!</button></div>
              </div>
            </motion.div>
          </motion.div>
        )}
        {showDiffUp && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
             <div className="bg-yellow-400 text-white font-black p-8 rounded-full shadow-2xl text-2xl animate-bounce">🔥 {showDiffUp} LEVEL! 🔥</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
